...
static void	ft_putdipos(long long int nbr, int len, int *arr)
{
	...
	(arr[0] == '+' ? 1 : 0) - (arr[0] == ' ' ? 1 : 0);
	arr[2] == -2 && nbr > 0 ? spaces-- : 0;
	padding = arr[2] - len;
	if (arr[0] != '-')
	{
		// no idea if i actually have to put arr[0] == '+' and arr[6]
		arr[0] == '0' && arr[2] <= 0 ? ft_putnchars(spaces, '0') : 0;
		arr[0] == '0' && arr[2] > 0 ? ft_putnchars(spaces, ' ') : 0;
		arr[0] == '0' && arr[2] > 0 && nbr == 0 ? ft_putnchars(spaces, '0') : 0;
		arr[2] != 0 && arr[2] != -2 && arr[0] != '0' ? ft_putnchars(spaces, ' ') : 0;
		arr[0] != '0' && (arr[0] == '+' || arr[6] == '+') ? ft_putchar('+') : 0;
		ft_putnchars(padding, '0');
		(arr[0] == '+' || arr[6] == '+') && arr[2] == 0 ? ft_putnchars(spaces, '0') : 0;
		(arr[0] == '+' || arr[6] == '+') && nbr == 0 ? ft_putllnbr(nbr) : 0;
		(arr[0] == -1 || arr[0] == ' ') && nbr == 0 && arr[2] != 0 ? ft_putchar('0') : 0;
	...
	(arr[2] == 0 && arr[0] != '+' && arr[6] == '+' && nbr == 0 ? 0 : len) +
	(arr[0] == '+' || arr[6] == '+' ? 1 : 0);
}
